...
func CreateProductFeeSettings(c echo.Context) error {
	var err error
	var status int
	...
	status, err = models.CreateProductFeeSettings(params, feeItems)
	if err != nil {
...
func UpdateProductFeeSettings(c echo.Context) error {
	var err error
	var status int
	...
	status, err = models.UpdateProductFeeSettings(params, feeItems)
	if err != nil {
		// log.Error("Failed create request data: " + err.Error())
		return lib.CustomError(status, err.Error(), "failed input data")
	}

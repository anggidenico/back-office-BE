...
func GetTransactionPaymentReport(c echo.Context) error {
	...
	items := []string{"trade_date", "investor_name", "investor_amount", "payment_date", "payment_method", "payment_channel",
		"settled_amount", "product_name"}
	...
	if orderBy != "" {
		...
		if found {
			...
			var orderByJoin string
			orderByJoin = "t.nav_date"
			if orderBy == "trade_date" {
				orderByJoin = "t.nav_date"
			} else if orderBy == "investor_name" {
				orderByJoin = "c.full_name"
			} else if orderBy == "investor_amount" {
				orderByJoin = "t.trans_amount"
			} else if orderBy == "payment_date" {
				orderByJoin = "ts.settle_date"
			} else if orderBy == "payment_method" {
				orderByJoin = "pm.lkp_name"
			} else if orderBy == "payment_channel" {
				orderByJoin = "pc.pchannel_name"
			} else if orderBy == "settled_amount" {
				orderByJoin = "ts.settle_nominal"
	...
	var totalData models.TcPaymentReportTotal
	...
	var responseData models.ResponseTcPaymentReport

	var trFormaterList []models.TcPaymentReport
	stringKosong := ""
	...
	for _, tr := range listTransaksi {
		...
		var trFormater models.TcPaymentReport
		trFormater.TransactionKey = tr.TransactionKey
		trFormater.TradeDate = tr.TradeDate
		trFormater.InvestorName = tr.InvestorName
		trFormater.InvestAmount = tr.InvestAmount.Truncate(2)
		trFormater.SubscriptionFee = tr.SubscriptionFee.Truncate(2)
		trFormater.ServiceFee = tr.ServiceFee.Truncate(2)
		trFormater.TotalPaid = tr.TotalPaid.Truncate(2)
		trFormater.PaymentDate = &stringKosong
		if tr.PaymentDate != nil {
			trFormater.PaymentDate = tr.PaymentDate
		}
		trFormater.PaymentMethod = &stringKosong
		if tr.PaymentMethod != nil {
			trFormater.PaymentMethod = tr.PaymentMethod
		}
		trFormater.PaymentChannel = &stringKosong
		if tr.PaymentChannel != nil {
			trFormater.PaymentChannel = tr.PaymentChannel
		}
		trFormater.ReferenceNo = &stringKosong
		if tr.ReferenceNo != nil {
			trFormater.ReferenceNo = tr.ReferenceNo
		}
		trFormater.AccountNo = &stringKosong
		if tr.AccountNo != nil {
			trFormater.AccountNo = tr.AccountNo
		}
		trFormater.SettledAmount = tr.SettledAmount.Truncate(2)
		trFormater.ProductName = &stringKosong
		if tr.ProductName != nil {
			trFormater.ProductName = tr.ProductName
		}
		trFormater.Remark = &stringKosong
		if tr.Remark != nil {
			trFormater.Remark = tr.Remark
		}
	...
	responseData.Data = &trFormaterList
	...
	//set header
	var header models.TcPaymentReportHeader
	if datefrom == dateto {
		date, _ := time.Parse(layout, datefrom)
		dateString = date.Format(dateLayout)
	} else {
		date, _ := time.Parse(layout, params["dateFrom"])
		dateFromString := date.Format(dateLayout)
		date, _ = time.Parse(layout, params["dateTo"])
		dateToString := date.Format(dateLayout)
		dateString = dateFromString + " - " + dateToString
	}

	header.ReportName = "Transaction Settlement Reports"
	header.Date = dateString
	header.SettlementDate = dateString
	header.Product = productName

	header.PaymentPartner = ""
	if totalData.PaymentChannel != nil {
		header.PaymentPartner = *totalData.PaymentChannel
	}

	header.AccNo = ""
	if totalData.AccountNo != nil {
		header.AccNo = *totalData.AccountNo
	}

	header.Bank = ""
	if totalData.BankName != nil {
		header.Bank = *totalData.BankName
	}

	header.SettlementAmount = zero
	if totalData.TotalSettledAmount.Cmp(zero) == 1 {
		header.SettlementAmount = totalData.TotalSettledAmount.Truncate(2)
	}

	//set total
	var total models.TcPaymentReportTotal
	total.TotalInvestAmount = zero
	total.TotalSubscriptionFee = zero
	total.TotalServiceFee = zero
	total.TotalTotalPaid = zero
	total.TotalSettledAmount = zero

	if totalData.TotalInvestAmount.Cmp(zero) == 1 {
		total.TotalInvestAmount = totalData.TotalInvestAmount.Truncate(2)
	}
	if totalData.TotalSubscriptionFee.Cmp(zero) == 1 {
		total.TotalSubscriptionFee = totalData.TotalSubscriptionFee.Truncate(2)
	}
	if totalData.TotalServiceFee.Cmp(zero) == 1 {
		total.TotalServiceFee = totalData.TotalServiceFee.Truncate(2)
	}
	if totalData.TotalTotalPaid.Cmp(zero) == 1 {
		total.TotalTotalPaid = totalData.TotalTotalPaid.Truncate(2)
	}
	if totalData.TotalSettledAmount.Cmp(zero) == 1 {
		total.TotalSettledAmount = totalData.TotalSettledAmount.Truncate(2)
	}
	...
	var pagination int
	if limit > 0 {
		if int(totalData.CountData) < int(limit) {
			pagination = 1
		} else {
			calc := math.Ceil(float64(countData.CountData) / float64(limit))
			pagination = int(calc)
...
func GetListCustomerBranchAgentDropdown(c echo.Context) error {

...
func getListAdminTransaction(c echo.Context, trType []string) error {
	...
	transSource := c.QueryParam("trans_source")
	if transSource != "" {
		transSourceCek, err := strconv.ParseUint(transSource, 10, 64)
...
func CreateTransactionSubscription(c echo.Context) error {
	...
	if promoCode != "" {
		err, enable, text, promoKeyRes := validatePromo(promoCode, customerKeyStr, productKeyStr, nil)
		if err != nil {
...
func UpdateTransactionSubscription(c echo.Context) error {
	var err error
	var status int
	decimal.MarshalJSONWithoutQuotes = true
	params := make(map[string]string)
	...
	promoCode := c.FormValue("promo_code")
	if promoCode != "" {
		err, enable, text, promoKeyRes := validatePromo(promoCode, customerKeyStr, productKeyStr, &transactionKey)
		if err != nil {
			return lib.CustomError(http.StatusBadRequest, err.Error(), "Failed get data")
		} else {
			if enable == false {
	...
	//save/update to promo used
	if promoKey != nil {
		var prom models.TrPromoUsed
		status, err = models.GetTrPromoUsedByField(&prom, "transaction_key", transactionKey)
		if err != nil {
			paramsPromoUsed := make(map[string]string)
			paramsPromoUsed["promo_used_key"] = strconv.FormatUint(prom.PromoUsedKey, 10)
			paramsPromoUsed["used_date"] = time.Now().Format(dateLayout)
			paramsPromoUsed["promo_key"] = *promoKey
			paramsPromoUsed["user_login_key"] = strIDUserLogin
			paramsPromoUsed["customer_key"] = customerKeyStr
			paramsPromoUsed["transaction_key"] = transactionKey
			paramsPromoUsed["used_status"] = "317"
			paramsPromoUsed["rec_status"] = "1"
			paramsPromoUsed["rec_modified_date"] = time.Now().Format(dateLayout)
	...
	//update tr_transaction_bank_account
	paramsBankTransaction := make(map[string]string)
	...
	settlementParams := make(map[string]string)
	settlementParams["settle_realized_date"] = time.Now().Format(dateLayout)
	settlementParams["transaction_key"] = transactionKey
	settlementParams["settle_purposed"] = "297"
	settlementParams["settle_date"] = navdate + " 00:00:00"
	settlementParams["settle_nominal"] = totalAmountStr
	settlementParams["client_subaccount_no"] = ""
	...
	SentEmailTransactionToBackOfficeAndSales(transactionKey, "11")
...
func UpdateTransactionRedemption(c echo.Context) error {
	var err error
	var status int
	decimal.MarshalJSONWithoutQuotes = true
	params := make(map[string]string)
...
func GetListAdminTransactionCorrection(c echo.Context) error {
	...
	items := []string{"transaction_key", "branch_key", "agent_key", "customer_key", "product_key", "trans_date", "trans_amount", "trans_source"}
	...
	var trTransaction []models.AdminTransactionCorrection

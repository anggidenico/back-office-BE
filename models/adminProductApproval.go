...
type ProductRequest struct {
	...
	ProductPhaseName     *string          `db:"product_phase_name"      json:"product_phase_name"`
	NavValuationType     *uint64          `db:"nav_valuation_type"      json:"nav_valuation_type"`
	NavValuationTypeName *string          `db:"nav_valuation_type_name" json:"nav_valuation_type_name"`
...
func GetProductRequestDetail(RecPK string) ProductUpdateDetails {
	...
	a1 := GetForeignKeyValue("ms_currency", "code", "currency_key", *result.Updates.CurrencyKey)
	result.Updates.CurrencyName = &a1

	a2 := GetForeignKeyValue("gen_lookup", "lkp_name", "lookup_key", *result.Updates.ProductPhase)
	result.Updates.ProductPhaseName = &a2

	a3 := GetForeignKeyValue("gen_lookup", "lkp_name", "lookup_key", *result.Updates.NavValuationType)
	result.Updates.NavValuationTypeName = &a3

	a4 := GetForeignKeyValue("ms_fund_type", "fund_type_name", "fund_type_key", *result.Updates.FundTypeKey)
	result.Updates.FundTypeName = &a4

	a5 := GetForeignKeyValue("ms_product_category", "category_name", "product_category_key", *result.Updates.ProductCategoryKey)
	result.Updates.ProductCategoryName = &a5
	...
	if result.Type == "UPDATE" {
		...
		// GET EXISTING DATA FROM MS_PRODUCT
	...
	FROM ms_product WHERE rec_status = 1 AND product_key = ` + strconv.FormatUint(*result.Updates.ProductKey, 10)

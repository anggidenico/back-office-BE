...
type CustomerBankAccountSinvest struct {
...
func GetCustomerBankAccountSinvest(c *[]CustomerBankAccountSinvest, customerKey string) (int, error) {
	query := `SELECT 
				b.bank_code,
				b.swift_code,
				b.bank_name,
			...
			FROM ms_customer_bank_account AS cb 
			INNER JOIN ms_bank_account AS ba ON ba.bank_account_key = cb.bank_account_key
			INNER JOIN ms_currency AS c ON c.currency_key = ba.currency_key 
			INNER JOIN ms_bank AS b ON b.bank_key = ba.bank_key 
			WHERE cb.rec_status = 1 AND ba.rec_status = 1 AND b.rec_status = 1 

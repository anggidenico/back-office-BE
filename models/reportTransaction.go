...
func AdminGetHeaderDailySubsRedmBatchForm(c *HeaderDailySubsRedmBatchForm, params map[string]string) (int, error) {
	query := `SELECT 
			CONCAT("` + config.ImageUrl + `", "/images/mail/report_logo_mnc.jpg") AS logo,
...
type ResponseTcPaymentReport struct {
	Header TcPaymentReportHeader `json:"header"`
	Data   *[]TcPaymentReport    `json:"data"`
	Total  TcPaymentReportTotal  `json:"total"`
}

type TcPaymentReport struct {
...
func TransactionPaymentReport(c *[]TcPaymentReport, limit uint64, offset uint64, params map[string]string, nolimit bool) (int, error) {
	dateFrom := ""
	dateTo := ""
	...
	query := `SELECT
				...
				DATE_FORMAT(t.nav_date, '%d %M %Y') AS trade_date,
				c.full_name AS investor_name, 
				t.trans_amount AS invest_amount,
				(t.trans_fee_amount + t.charges_fee_amount) AS subscription_fee,
				t.services_fee_amount AS service_fee,
				t.total_amount AS total_paid,
				DATE_FORMAT(ts.settle_date, '%d %M %Y %H:%i %p') AS payment_date,
				pm.lkp_name AS payment_method,
				pc.pchannel_name AS payment_channel,
				ts.settle_references AS reference_no,
				ts.client_subaccount_no AS account_no,
				ts.settle_nominal AS settled_amount,
				p.product_name_alt AS product_name,
				"" AS remark 
			FROM tr_transaction AS t 
			INNER JOIN ms_customer AS c ON c.customer_key = t.customer_key 
			...
			LEFT JOIN ms_payment_channel AS pc ON pc.pchannel_key = ts.settle_payment_method 
			LEFT JOIN ms_product_channel AS proc ON proc.product_key = t.product_key AND proc.pchannel_key = pc.pchannel_key 
			LEFT JOIN ms_bank_account AS ba ON ba.bank_account_key = proc.bank_account_key 
			LEFT JOIN ms_bank AS b ON b.bank_key = ba.bank_key 
			WHERE t.rec_status = 1 AND t.trans_type_key = 1`
...
type TcPaymentReportTotal struct {
	CountData            int             `db:"count_data"                   json:"count_data,omitempty"`
	TotalInvestAmount    decimal.Decimal `db:"total_invest_amount"          json:"total_invest_amount"`
	TotalSubscriptionFee decimal.Decimal `db:"total_subscription_fee"       json:"total_subscription_fee"`
	TotalServiceFee      decimal.Decimal `db:"total_service_fee"            json:"total_service_fee"`
	TotalTotalPaid       decimal.Decimal `db:"total_paid"                   json:"total_paid"`
	TotalSettledAmount   decimal.Decimal `db:"total_settled_amount"         json:"total_settled_amount"`
...
type TcPaymentReportHeader struct {
	ReportName       string          `json:"report_name"`
	Date             string          `json:"date"`
	PaymentPartner   string          `json:"payment_partner"`
...
func TransactionPaymentReportTotal(c *TcPaymentReportTotal, params map[string]string) (int, error) {
	dateFrom := ""
	dateTo := ""
	...
	query := `SELECT
				COUNT(t.transaction_key) AS count_data,
				(CASE
					WHEN SUM(t.trans_amount) IS NOT NULL THEN SUM(t.trans_amount)
					ELSE 0
				END) AS total_invest_amount,
				(CASE
					WHEN SUM(t.trans_fee_amount) IS NOT NULL THEN SUM(t.trans_fee_amount)
					ELSE 0
				END) AS total_subscription_fee,
				(CASE
					WHEN (SUM(t.charges_fee_amount) + SUM(t.services_fee_amount)) IS NOT NULL THEN (SUM(t.charges_fee_amount) + SUM(t.services_fee_amount))
					ELSE 0
				END) AS total_service_fee,
				(CASE
					WHEN SUM(t.total_amount) IS NOT NULL THEN SUM(t.total_amount)
					ELSE 0
				END) AS total_paid,
				(CASE
					WHEN SUM(ts.settle_nominal) IS NOT NULL THEN SUM(ts.settle_nominal)
					ELSE 0

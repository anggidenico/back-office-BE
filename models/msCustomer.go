...
func GetCustomerDropdown(c *[]CustomerDropdown, params map[string]string, paramsLike map[string]string) (int, error) {
	...
	query := `SELECT 
			...
			FROM ms_customer AS c
			WHERE c.rec_status = 1 AND c.investor_type IN (263, 264)`
...
func GetHeaderCustomerDetailAccountStatement(c *HeaderCustomerDetailAccountStatement, customerKey string) (int, error) {
	query := `SELECT 
			...
			WHERE oa.oa_status IN (260, 261, 262) AND oa.customer_key = '` + customerKey + `'
...
type HeaderCustomerInstitution struct {
	CustomerKey uint64  `db:"customer_key"                json:"customer_key"`
	Cif         *string `db:"cif"                         json:"cif"`
	SidNo       *string `db:"sid"                         json:"sid"`
	FullName    *string `db:"full_name"                   json:"full_name"`
...
func AdminGetHeaderDetailCustomerInstitution(c *HeaderCustomerInstitution, customerKey string) (int, error) {
	query := `SELECT
				c.customer_key,
				c.unit_holder_idno AS cif,
				c.sid_no AS sid,
				c.full_name,
				pa.address_line1 AS alamat,
				ci.city_name AS city,
				co.cou_name AS country 
			FROM ms_customer AS c 
			INNER JOIN oa_request AS o ON o.customer_key = c.customer_key 
			INNER JOIN oa_institution_data AS i ON i.oa_request_key = i.oa_request_key 
			LEFT JOIN ms_country AS co ON co.country_key = i.nationality 
			LEFT JOIN oa_postal_address AS pa ON pa.postal_address_key = i.correspondence_key 
...
func GetCustomerBranchAgentDropdown(c *[]CustomerDropdown, branchKey string, agentKey string, customerName string) (int, error) {

	cusCondition := ""
	if customerName != "" {
		cusCondition = " AND c.full_name like '%" + customerName + "%' "
	}

	cusAgent := ""
	if agentKey != "" {
		cusAgent = " AND mab.agent_key = '" + agentKey + "'"
	}

	cusBranch := ""
	if branchKey != "" {
		cusBranch = " AND mab.branch_key = '" + branchKey + "'"
	}

	query := `SELECT 
				c.customer_key AS customer_key,
				CONCAT(c.unit_holder_idno, " - ", c.full_name) AS name,
				mab.branch_key AS branch_key,
				mab.agent_key AS agent_key 
			FROM ms_customer AS c
			INNER JOIN (
					SELECT MAX(eff_date) AS eff_date, agent_key, customer_key  
					FROM ms_agent_customer 
					WHERE rec_status = 1
					GROUP BY customer_key 
					ORDER BY eff_date DESC
				)  AS ag ON ag.customer_key = c.customer_key 
			INNER JOIN ms_agent_customer AS mac ON mac.eff_date = ag.eff_date AND mac.customer_key = c.customer_key AND mac.rec_status = 1 
			INNER JOIN (
					SELECT MAX(eff_date) AS eff_date, agent_key, branch_key  
					FROM ms_agent_branch 
					WHERE rec_status = 1
					GROUP BY agent_key 
					ORDER BY eff_date DESC
				)  AS ab ON ab.agent_key = mac.agent_key 
			INNER JOIN ms_agent_branch AS mab ON mab.eff_date = ab.eff_date AND mab.agent_key = ab.agent_key  AND mab.rec_status = 1 
			WHERE c.rec_status = 1 AND c.investor_type IN (263, 264) 
			` + cusBranch + cusAgent + cusCondition + `
...
func UpdateMsCustomerByField(params map[string]string, value string, field string) (int, error) {
	query := "UPDATE ms_customer SET "
	// Get params
	i := 0
	for key, value := range params {

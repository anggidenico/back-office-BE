...
func GetLastAgentCustomer(c *MsAgentCustomer, customerKey string) (int, error) {
	query2 := `SELECT t1.agent_customer_key, t1.agent_key, t1.customer_key, t1.eff_date, t1.ref_code_used FROM
			   ms_agent_customer t1 JOIN (SELECT MAX(agent_customer_key) agent_customer_key FROM ms_agent_customer WHERE customer_key = "` + customerKey + `" AND eff_date <= NOW() GROUP BY customer_key) t2
